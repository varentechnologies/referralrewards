...
public class MenuControllers {
  ...
  public String processInfo(Model model, @AuthenticationPrincipal DefaultUser user) {
    if (user.getAuthorities().contains(new SimpleGrantedAuthority("recruiter")) || user
        .getAuthorities().contains(new SimpleGrantedAuthority("admin")) || user.getAuthorities()
        .contains(new SimpleGrantedAuthority("superadmin"))) {
